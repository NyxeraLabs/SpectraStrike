...
"""Impacket psexec wrapper with standardized SDK telemetry contracts."""
...
logger = get_logger("spectrastrike.wrappers.impacket.psexec")


class ImpacketPsexecError(RuntimeError):
    """Raised when impacket psexec execution fails."""


@dataclass(slots=True, frozen=True)
class ImpacketPsexecRequest:
    """Impacket psexec normalized request model."""
...
@dataclass(slots=True, frozen=True)
class ImpacketPsexecResult:
    """Normalized impacket psexec execution output."""
...
class ImpacketPsexecWrapper(BaseWrapper):
    """Wrapper around impacket psexec.py with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("IMPACKET_PSEXEC_BINARY", "psexec.py")
        super().__init__(
    ...
    def detect_version(self) -> str:
        """Detect impacket psexec version."""
        return self.detect_tool_version([["--version"], ["-version"], ["-h"]])

    @staticmethod
    def _is_dry_run(request: ImpacketPsexecRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: ImpacketPsexecRequest) -> list[str]:
        """Build deterministic psexec command."""
    ...
    def execute(
        self,
        request: ImpacketPsexecRequest,
        *,
        tenant_id: str,
        operator_id: str,
    ) -> ImpacketPsexecResult:
        """Execute psexec and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise ImpacketPsexecError(
                    result_output or "impacket psexec command failed"
                )
    ...
    def send_to_orchestrator(
        self,
        result: ImpacketPsexecResult,
        *,
        telemetry: TelemetryIngestionPipeline,
        tenant_id: str,
        operator_id: str,
        actor: str = "impacket-psexec-wrapper",
    ) -> TelemetryEvent:
        ...
        try:
            ...
            raise ImpacketPsexecError(str(exc)) from exc
