...
"""Impacket secretsdump wrapper with standardized SDK telemetry contracts."""
...
logger = get_logger("spectrastrike.wrappers.impacket.secretsdump")


class ImpacketSecretsdumpError(RuntimeError):
    """Raised when impacket secretsdump execution fails."""


@dataclass(slots=True, frozen=True)
class ImpacketSecretsdumpRequest:
    """Impacket secretsdump normalized request model."""

    target: str
    username: str
    command: str = "-just-dc-user administrator"
    ...
    def __post_init__(self) -> None:
        ...
        if not (self.password or self.hashes or self.no_pass):
            raise ValueError("one auth mode is required: password, hashes, or no_pass")
        if not self.policy_decision_hash.strip():
            raise ValueError("policy_decision_hash is required")


@dataclass(slots=True, frozen=True)
class ImpacketSecretsdumpResult:
    """Normalized impacket secretsdump execution output."""
...
class ImpacketSecretsdumpWrapper(BaseWrapper):
    """Wrapper around impacket secretsdump.py with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("IMPACKET_SECRETSDUMP_BINARY", "secretsdump.py")
        super().__init__(
    ...
    def detect_version(self) -> str:
        """Detect impacket secretsdump version."""
        return self.detect_tool_version([["--version"], ["-version"], ["-h"]])

    @staticmethod
    def _is_dry_run(request: ImpacketSecretsdumpRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: ImpacketSecretsdumpRequest) -> list[str]:
        """Build deterministic secretsdump command."""
        identity = request.username
        if request.domain:
            identity = f"{request.domain}/{identity}"
        if request.password and not request.no_pass:
            identity = f"{identity}:{request.password}"
        target = f"{identity}@{request.target}"

        command = [self._binary, target, *request.command.split()]
        if request.hashes:
            command.extend(["-hashes", request.hashes])
        if request.no_pass:
    ...
    def execute(
        ...
        *,
        ...
        """Execute secretsdump and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise ImpacketSecretsdumpError(
                    result_output or "impacket secretsdump command failed"
                )
        ...
        return ImpacketSecretsdumpResult(
            target=request.target,
    ...
    def send_to_orchestrator(
        self,
        result: ImpacketSecretsdumpResult,
        *,
        telemetry: TelemetryIngestionPipeline,
        tenant_id: str,
        operator_id: str,
        actor: str = "impacket-secretsdump-wrapper",
    ) -> TelemetryEvent:
        ...
        try:
            ...
            raise ImpacketSecretsdumpError(str(exc)) from exc
