...
"""BloodHound collector wrapper with standardized SDK telemetry contracts."""
...
logger = get_logger("spectrastrike.wrappers.bloodhound.collector")


class BloodhoundCollectorError(RuntimeError):
    """Raised when BloodHound collector execution fails."""


@dataclass(slots=True, frozen=True)
class BloodhoundCollectorRequest:
    """BloodHound collector normalized request model."""
    ...
    def __post_init__(self) -> None:
        ...
        if not (self.password or self.no_pass):
            raise ValueError("one auth mode is required: password or no_pass")
        if not self.policy_decision_hash.strip():
            raise ValueError("policy_decision_hash is required")


@dataclass(slots=True, frozen=True)
class BloodhoundCollectorResult:
    """Normalized BloodHound collector execution output."""
...
class BloodhoundCollectorWrapper(BaseWrapper):
    """Wrapper around BloodHound collector with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("BLOODHOUND_COLLECTOR_BINARY", "bloodhound-python")
        super().__init__(
            tool_name="bloodhound-collector",
    ...
    def detect_version(self) -> str:
        """Detect BloodHound collector version."""
        return self.detect_tool_version([["--version"], ["-h"]])

    @staticmethod
    def _is_dry_run(request: BloodhoundCollectorRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: BloodhoundCollectorRequest) -> list[str]:
        """Build deterministic BloodHound collector command."""
        command = [self._binary, *request.command.split(), "-ns", request.target, "-u", request.username]
        if request.domain:
            command.extend(["-d", request.domain])
        if request.password and not request.no_pass:
            command.extend(["-p", request.password])
        if request.no_pass:
    ...
    def execute(
        ...
        *,
        ...
        """Execute collector and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise BloodhoundCollectorError(
                    result_output or "bloodhound collector command failed"
                )

        signature_payload = {
            "tool": "bloodhound-collector",
            "version": version,
            "target": request.target,
            "operator_id": operator_id,
            "tenant_id": tenant_id,
        ...
        return BloodhoundCollectorResult(
            target=request.target,
    ...
    def send_to_orchestrator(
        self,
        result: BloodhoundCollectorResult,
        *,
        telemetry: TelemetryIngestionPipeline,
        tenant_id: str,
        operator_id: str,
        actor: str = "bloodhound-collector-wrapper",
    ) -> TelemetryEvent:
        ...
        try:
            ...
            raise BloodhoundCollectorError(str(exc)) from exc
