...
"""SCP wrapper with standardized SDK telemetry contracts."""
...
class ScpExecutionError(RuntimeError):
    """Raised when scp execution fails."""


@dataclass(slots=True, frozen=True)
class ScpRequest:
    """SCP normalized request model."""

    target: str
    command: str = "/tmp/scp_smoke_src.txt /tmp/scp_smoke_dst.txt"
...
@dataclass(slots=True, frozen=True)
class ScpResult:
    """Normalized scp execution output."""
...
class ScpWrapper(BaseWrapper):
    """Wrapper around scp with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("SCP_BINARY", "scp")
        super().__init__(
            tool_name="scp",
    ...
    def detect_version(self) -> str:
        """Detect scp version using ssh sibling version banner."""
        ssh_binary = os.getenv("SSH_BINARY", "ssh")
        try:
            return self.detect_tool_version_with_binary(
                ssh_binary,
                [["-V"], ["-h"], ["--help"]],
            )
        except Exception:
            return self.detect_tool_version([["-h"], ["--help"]])

    @staticmethod
    def _is_dry_run(request: ScpRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: ScpRequest) -> list[str]:
        """Build deterministic scp command."""
    ...
    def execute(
        ...
        *,
        ...
        """Execute scp and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise ScpExecutionError(result_output or "scp command failed")
    ...
    def send_to_orchestrator(
        ...
        *,
        ...
        try:
            ...
            raise ScpExecutionError(str(exc)) from exc
