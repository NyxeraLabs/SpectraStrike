...
"""Impacket smbexec wrapper with standardized SDK telemetry contracts."""
...
logger = get_logger("spectrastrike.wrappers.impacket.smbexec")


class ImpacketSmbexecError(RuntimeError):
    """Raised when impacket smbexec execution fails."""


@dataclass(slots=True, frozen=True)
class ImpacketSmbexecRequest:
    """Impacket smbexec normalized request model."""
...
@dataclass(slots=True, frozen=True)
class ImpacketSmbexecResult:
    """Normalized impacket smbexec execution output."""
...
class ImpacketSmbexecWrapper(BaseWrapper):
    """Wrapper around impacket smbexec.py with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("IMPACKET_SMBEXEC_BINARY", "smbexec.py")
        super().__init__(
    ...
    def detect_version(self) -> str:
        """Detect impacket smbexec version."""
        return self.detect_tool_version([["--version"], ["-version"], ["-h"]])

    @staticmethod
    def _is_dry_run(request: ImpacketSmbexecRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: ImpacketSmbexecRequest) -> list[str]:
        """Build deterministic smbexec command."""
    ...
    def execute(
        ...
        *,
        ...
        """Execute smbexec and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise ImpacketSmbexecError(
                    result_output or "impacket smbexec command failed"
                )
    ...
    def send_to_orchestrator(
        self,
        result: ImpacketSmbexecResult,
        *,
        telemetry: TelemetryIngestionPipeline,
        tenant_id: str,
        operator_id: str,
        actor: str = "impacket-smbexec-wrapper",
    ) -> TelemetryEvent:
        ...
        try:
            ...
            raise ImpacketSmbexecError(str(exc)) from exc
