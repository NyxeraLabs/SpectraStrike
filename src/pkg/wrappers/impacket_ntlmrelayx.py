...
"""Impacket ntlmrelayx wrapper with standardized SDK telemetry contracts."""
...
logger = get_logger("spectrastrike.wrappers.impacket.ntlmrelayx")


class ImpacketNtlmrelayxError(RuntimeError):
    """Raised when impacket ntlmrelayx execution fails."""


@dataclass(slots=True, frozen=True)
class ImpacketNtlmrelayxRequest:
    """Impacket ntlmrelayx normalized request model."""

    target: str
    username: str
    command: str = "-t smb://127.0.0.1 -smb2support"
...
@dataclass(slots=True, frozen=True)
class ImpacketNtlmrelayxResult:
    """Normalized impacket ntlmrelayx execution output."""
...
class ImpacketNtlmrelayxWrapper(BaseWrapper):
    """Wrapper around impacket ntlmrelayx.py with SDK telemetry contract enforcement."""
    ...
    def __init__(
        ...
        *,
        ...
        self._binary = binary or os.getenv("IMPACKET_NTLMRELAYX_BINARY", "ntlmrelayx.py")
        super().__init__(
    ...
    def detect_version(self) -> str:
        """Detect impacket ntlmrelayx version."""
        return self.detect_tool_version([["--version"], ["-version"], ["-h"]])

    @staticmethod
    def _is_dry_run(request: ImpacketNtlmrelayxRequest) -> bool:
        return any(arg.strip().lower() == "--dry-run" for arg in request.extra_args)

    def build_command(self, request: ImpacketNtlmrelayxRequest) -> list[str]:
        """Build deterministic ntlmrelayx command."""
    ...
    def execute(
        ...
        *,
        ...
        """Execute ntlmrelayx and produce signed normalized output contract."""
        ...
        if self._is_dry_run(request):
            ...
            if completed.returncode != 0:
                raise ImpacketNtlmrelayxError(
                    result_output or "impacket ntlmrelayx command failed"
                )
        ...
        return ImpacketNtlmrelayxResult(
            target=request.target,
    ...
    def send_to_orchestrator(
        self,
        result: ImpacketNtlmrelayxResult,
        *,
        telemetry: TelemetryIngestionPipeline,
        tenant_id: str,
        operator_id: str,
        actor: str = "impacket-ntlmrelayx-wrapper",
    ) -> TelemetryEvent:
        ...
        try:
            ...
            raise ImpacketNtlmrelayxError(str(exc)) from exc
