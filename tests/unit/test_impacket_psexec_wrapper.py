...
"""Unit tests for Impacket psexec wrapper standardized SDK contract."""
...
def test_build_command_includes_identity_and_auth_flags() -> None:
    ...
    assert command[0] == "psexec.py"
    assert command[1].startswith("ACME/administrator:pass123@10.0.0.50")
...
def test_execute_raises_on_non_zero_status() -> None:
    ...
    with pytest.raises(ImpacketPsexecError, match="SMB SessionError"):
        wrapper.execute(
            ImpacketPsexecRequest(
                target="10.10.10.1",
                username="administrator",
