...
"""Unit tests for netexec wrapper standardized SDK contract."""
...
def test_build_command_contains_protocol_target_and_flags() -> None:
    wrapper = NetExecWrapper(binary="nxc", signer=lambda _payload: "sig")
    ...
    assert command[0] == "nxc"
    assert "smb" in command
    assert "127.0.0.1" in command
    assert "--shares" in command
    assert "--timeout" in command and "5" in command
...
def test_detect_version_parses_banner() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "NetExec v1.3.0"
            stderr = ""

        return _R()

    wrapper = NetExecWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "1.3.0"
...
def test_execute_builds_contract_fields() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "SMB 127.0.0.1:445 SUCCESS"
            stderr = ""
    ...
    wrapper = NetExecWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        NetExecRequest(target="127.0.0.1", command="smb 127.0.0.1 -u smoke -p smoke --shares"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = NetExecWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
...
def test_execute_raises_on_failure() -> None:
    ...
    wrapper = NetExecWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(NetExecExecutionError, match="auth failed"):
        wrapper.execute(
            NetExecRequest(target="127.0.0.1", command="smb 127.0.0.1 -u smoke -p smoke --shares"),
            tenant_id="tenant-a",
            operator_id="operator-a",
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    result = wrapper.execute(
        NetExecRequest(target="127.0.0.1", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
