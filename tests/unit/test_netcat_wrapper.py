...
"""Unit tests for netcat wrapper standardized SDK contract."""
...
def test_build_command_contains_target_and_port() -> None:
    wrapper = NetcatWrapper(binary="nc", signer=lambda _payload: "sig")
    ...
    assert command[0] == "nc"
    assert "-vz" in command
    assert "127.0.0.1" in command
    assert "80" in command
    assert "-w" in command and "3" in command
...
def test_detect_version_parses_banner() -> None:
    ...
    wrapper = NetcatWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "1.219"
...
def test_execute_builds_contract_fields() -> None:
    ...
    wrapper = NetcatWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        NetcatRequest(target="127.0.0.1", command="-vz 127.0.0.1 80"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = NetcatWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
...
def test_execute_raises_on_failure() -> None:
    ...
    wrapper = NetcatWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(NetcatExecutionError, match="connection refused"):
        wrapper.execute(
            NetcatRequest(target="127.0.0.1", command="-vz 127.0.0.1 80"),
            tenant_id="tenant-a",
            operator_id="operator-a",
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    result = wrapper.execute(
        NetcatRequest(target="127.0.0.1", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
