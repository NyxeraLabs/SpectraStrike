...
"""Unit tests for Impacket wmiexec wrapper standardized SDK contract."""
...
def test_build_command_includes_identity_and_auth_flags() -> None:
    wrapper = ImpacketWmiexecWrapper(
        binary="wmiexec.py",
    ...
    assert command[0] == "wmiexec.py"
    assert command[1].startswith("ACME/administrator:pass123@10.0.0.60")
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    assert event.event_type == "impacket_wmiexec_completed"
...
def test_execute_raises_on_non_zero_status() -> None:
    ...
    with pytest.raises(ImpacketWmiexecError, match="WMI SessionError"):
        wrapper.execute(
            ImpacketWmiexecRequest(
                target="10.10.10.1",
                username="administrator",
