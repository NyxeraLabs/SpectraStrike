...
"""Unit tests for gobuster wrapper standardized SDK contract."""
...
def test_build_command_contains_required_flags() -> None:
    wrapper = GobusterWrapper(binary="gobuster", signer=lambda _payload: "sig")
    request = GobusterScanRequest(
        target="http://127.0.0.1",
        command="dir -u http://127.0.0.1 -w words.txt -q",
        extra_args=["--no-error"],
    )
    command = wrapper.build_command(request)
    assert command[0] == "gobuster"
...
def test_detect_version_parses_banner() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "Gobuster v3.7.0"
            stderr = ""

        return _R()

    wrapper = GobusterWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "3.7.0"
...
def test_execute_builds_contract_fields() -> None:
    ...
    wrapper = GobusterWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        GobusterScanRequest(target="http://127.0.0.1", command="dir -u http://127.0.0.1 -w words.txt"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = GobusterWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
...
def test_execute_raises_on_failure() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 1
            stdout = ""
            stderr = "invalid wordlist path"

        return _R()

    wrapper = GobusterWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(GobusterExecutionError, match="invalid wordlist path"):
        wrapper.execute(
            GobusterScanRequest(target="http://127.0.0.1", command="dir -u http://127.0.0.1 -w missing.txt"),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    result = wrapper.execute(
        GobusterScanRequest(target="http://127.0.0.1", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
