...
"""Unit tests for curl wrapper standardized SDK contract."""
...
from pkg.wrappers.curl import CurlExecutionError, CurlRequest, CurlWrapper


def test_build_command_contains_expected_flags() -> None:
    wrapper = CurlWrapper(binary="curl", signer=lambda _payload: "sig")
    request = CurlRequest(
...
def test_detect_version_parses_banner() -> None:
    ...
    wrapper = CurlWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "8.18.0"
...
def test_execute_builds_contract_fields() -> None:
    ...
    wrapper = CurlWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        CurlRequest(target="http://127.0.0.1", command="--version"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = CurlWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
...
def test_execute_raises_on_failure() -> None:
    ...
    wrapper = CurlWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(CurlExecutionError, match="bad usage"):
        wrapper.execute(
            CurlRequest(target="http://127.0.0.1", command="-bad"),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    result = wrapper.execute(
        CurlRequest(target="http://127.0.0.1", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
