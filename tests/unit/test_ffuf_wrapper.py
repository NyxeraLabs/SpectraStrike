...
"""Unit tests for ffuf wrapper standardized SDK contract."""
...
def test_build_command_contains_required_flags() -> None:
    wrapper = FfufWrapper(binary="ffuf", signer=lambda _payload: "sig")
    request = FfufScanRequest(
        target="http://127.0.0.1/FUZZ",
        command="-u http://127.0.0.1/FUZZ -w words.txt -mc all -fc 404",
        extra_args=["-t", "5"],
    )
    command = wrapper.build_command(request)
    assert command[0] == "ffuf"
    assert "-u" in command and "http://127.0.0.1/FUZZ" in command
    assert "-w" in command and "words.txt" in command
    assert "-t" in command and "5" in command
...
def test_detect_version_parses_banner() -> None:
    ...
    wrapper = FfufWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "2.1.0"


def test_execute_builds_contract_fields() -> None:
    def fake_runner(command: list[str], _timeout: float):
        if "-V" in command or "-version" in command or "-h" in command:
        ...
        class _R:
            returncode = 0
            stdout = "[Status: 200, Size: 123, Words: 11, Lines: 2] /admin"
            stderr = ""
    ...
    wrapper = FfufWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        FfufScanRequest(
            target="http://127.0.0.1/FUZZ",
            command="-u http://127.0.0.1/FUZZ -w words.txt",
        ),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = FfufWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
    ...
    assert event.event_type == "ffuf_scan_completed"
...
def test_execute_raises_on_failure() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 1
            stdout = ""
            stderr = "error: wordlist missing"

        return _R()

    wrapper = FfufWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(FfufExecutionError, match="wordlist missing"):
        wrapper.execute(
            FfufScanRequest(
                target="http://127.0.0.1/FUZZ",
                command="-u http://127.0.0.1/FUZZ -w missing.txt",
            ),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    wrapper = FfufWrapper(
        ...
        runner=lambda _cmd, _timeout: type(
            "R", (), {"returncode": 0, "stdout": "ffuf version: 2.1.0-dev", "stderr": ""}
        )(),  # type: ignore[arg-type]
    )
    result = wrapper.execute(
        FfufScanRequest(target="http://127.0.0.1/FUZZ", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
