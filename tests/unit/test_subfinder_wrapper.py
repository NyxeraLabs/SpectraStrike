...
"""Unit tests for subfinder wrapper standardized SDK contract."""
...
def test_build_command_contains_domain_flags() -> None:
    wrapper = SubfinderWrapper(binary="subfinder", signer=lambda _payload: "sig")
...
def test_detect_version_parses_banner() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "subfinder v2.7.6"
            stderr = ""

        return _R()

    wrapper = SubfinderWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "2.7.6"
...
def test_execute_builds_contract_fields() -> None:
    ...
    wrapper = SubfinderWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = SubfinderWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
    ...
    assert event.event_type == "subfinder_scan_completed"
...
def test_execute_raises_on_failure() -> None:
    ...
    wrapper = SubfinderWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(SubfinderExecutionError, match="configuration error"):
        wrapper.execute(
            SubfinderRequest(target="example.com", command="-d example.com -silent"),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    wrapper = SubfinderWrapper(
        ...
        runner=lambda _cmd, _timeout: type(
            "R", (), {"returncode": 0, "stdout": "subfinder v2.7.6", "stderr": ""}
        )(),  # type: ignore[arg-type]
    )
    result = wrapper.execute(
        SubfinderRequest(target="example.com", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
