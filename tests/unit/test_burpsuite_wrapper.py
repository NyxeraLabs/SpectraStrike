...
"""Unit tests for burpsuite wrapper standardized SDK contract."""
...
def test_build_command_contains_help_flags() -> None:
    wrapper = BurpSuiteWrapper(binary="burpsuite", signer=lambda _payload: "sig")
    request = BurpSuiteRequest(
        target="local-burp",
        command="--help",
        extra_args=["--debug"],
    )
    command = wrapper.build_command(request)
    assert command[0] == "burpsuite"
...
def test_detect_version_parses_banner() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "Burp Suite Professional 2024.1.3"
            stderr = ""

        return _R()

    wrapper = BurpSuiteWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "2024.1.3"
...
def test_execute_builds_contract_fields() -> None:
    ...
    wrapper = BurpSuiteWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        BurpSuiteRequest(target="local-burp", command="--help"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = BurpSuiteWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
    ...
    assert event.event_type == "burpsuite_session_completed"
...
def test_execute_raises_on_failure() -> None:
    ...
    wrapper = BurpSuiteWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(BurpSuiteExecutionError, match="startup failed"):
        wrapper.execute(
            BurpSuiteRequest(target="local-burp", command="--headless"),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    wrapper = BurpSuiteWrapper(
        ...
        runner=lambda _cmd, _timeout: type(
            "R", (), {"returncode": 0, "stdout": "Burp Suite Professional 2024.1.3", "stderr": ""}
        )(),  # type: ignore[arg-type]
    )
    result = wrapper.execute(
        BurpSuiteRequest(target="local-burp", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
