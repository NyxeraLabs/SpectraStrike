...
"""Unit tests for john wrapper standardized SDK contract."""
...
def test_build_command_contains_wordlist_and_hash_file() -> None:
    wrapper = JohnWrapper(binary="john", signer=lambda _payload: "sig")
    request = JohnRequest(
        target="/tmp/hash.txt",
        command="--wordlist=/tmp/wl.txt /tmp/hash.txt --format=raw-md5",
        extra_args=["--session=s1"],
    )
    command = wrapper.build_command(request)
    assert command[0] == "john"
    assert "--wordlist=/tmp/wl.txt" in command
    assert "/tmp/hash.txt" in command
...
def test_detect_version_parses_banner() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 0
            stdout = "John the Ripper 1.9.0-jumbo-1"
            stderr = ""

        return _R()

    wrapper = JohnWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    assert wrapper.detect_version() == "1.9.0"


def test_execute_builds_contract_fields() -> None:
    def fake_runner(command: list[str], _timeout: float):
        if "--list=build-info" in command or "--list=help" in command or "--help" in command:
    ...
    wrapper = JohnWrapper(runner=fake_runner, signer=fake_signer)  # type: ignore[arg-type]
    result = wrapper.execute(
        JohnRequest(target="/tmp/hash.txt", command="--wordlist=/tmp/wl.txt /tmp/hash.txt"),
        tenant_id="tenant-a",
...
def test_send_to_orchestrator_emits_validated_payload() -> None:
    ...
    wrapper = JohnWrapper(runner=fake_runner, signer=lambda _payload: "sig-ed25519")  # type: ignore[arg-type]
...
def test_execute_raises_on_failure() -> None:
    def fake_runner(command: list[str], _timeout: float):
        ...
        class _R:
            returncode = 1
            stdout = ""
            stderr = "invalid hash file"

        return _R()

    wrapper = JohnWrapper(runner=fake_runner, signer=lambda _payload: "sig")  # type: ignore[arg-type]
    with pytest.raises(JohnExecutionError, match="invalid hash file"):
        wrapper.execute(
            JohnRequest(target="/tmp/missing.hash", command="/tmp/missing.hash"),
            tenant_id="tenant-a",
            operator_id="operator-a",
        )
...
def test_send_to_orchestrator_supports_ingest_payload_compatible_sink() -> None:
    ...
    result = wrapper.execute(
        JohnRequest(target="/tmp/hash.txt", extra_args=["--dry-run"]),
        tenant_id="tenant-a",
        operator_id="operator-a",
    )
