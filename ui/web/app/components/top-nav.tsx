...
import { GlobalUiControls } from "./global-ui-controls";
...
export function TopNav() {
  ...
  return (
    <nav className="spectra-panel flex flex-col items-start gap-3 px-4 py-3">
      ...
      <ul className="flex w-full flex-wrap items-center gap-2">
        {navItems.map((item) => (
          <li key={item.href}>
            <Link href={item.href} className="spectra-button-secondary inline-flex px-3 py-2 text-xs font-semibold">
              {item.label}
            </Link>
          </li>
        ))}
